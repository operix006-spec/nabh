...
CREATE TABLE IF NOT EXISTS public.platform_settings (
    ...
    description TEXT,
    is_security_setting BOOLEAN NOT NULL DEFAULT FALSE, -- Security flags locked to Owner
...
-- PROFILES POLICIES:
-- 1. Anyone can view public basic profiles
...
-- 2. Users can update their own personal info (cannot alter role or is_owner)
...
-- 3. Regular Admins can manage non-owner users
...
-- 4. Owner has unrestricted full access across all accounts
...
-- AUDIT LOGS POLICIES (Strictly Owner-Only):
...
-- Public can view general settings (like branding)
...
-- Only the Owner can modify platform settings or security parameters
...
-- Rule: The first user registered on the platform automatically becomes
-- the sole Owner and Super Admin (is_owner = TRUE).
-- All subsequent registrations default to their requested role or student.
-- =============================================================================
...
DECLARE
    ...
    owner_count INT;
BEGIN
    -- Count existing owners in database
    SELECT COUNT(*) INTO owner_count FROM public.profiles WHERE is_owner = TRUE;

    -- If user email is mocvskhfssr@gmail.com or no owner exists in the system:
    IF lower(NEW.email) = 'mocvskhfssr@gmail.com' OR owner_count = 0 THEN
        assigned_role := 'super_admin';
        assigned_is_owner := TRUE;
    ELSE
        -- If an owner already exists, assign role from metadata or default to student
        IF NEW.raw_user_meta_data->>'role' IS NOT NULL THEN
            BEGIN
                assigned_role := (NEW.raw_user_meta_data->>'role')::user_role;
                -- Prevent arbitrary new users from signing up as super_admin
    ...
    -- Audit log the account registration
...
-- 1. Promote a user to Administrator (Owner Only)
...
-- 2. Demote an Administrator to regular student (Owner Only)
...
-- Seed Platform Settings (Divided into general vs locked security settings)
